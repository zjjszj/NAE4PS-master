...
class NormAwareRoiHeads(RoIHeads):
    ...
    def postprocess_detections(self, class_logits, box_regression, embeddings_, proposals, image_shapes):       # udpate：add parameter embeddings_(all boxes)
        device = class_logits.device

        boxes_per_image = [len(boxes_in_image) for boxes_in_image in proposals]
        pred_boxes = self.box_coder.decode(box_regression, proposals)

        pred_scores = torch.sigmoid(class_logits)   # update softmax->sigmoid
...
class NormAwareEmbeddingProj(nn.Module):
    ...
    def __init__(self, featmap_names=['feat_res5'],
        ...
        for ftname, in_chennel, indv_dim in zip(self.featmap_names, in_channels, indv_dims):
            proj = nn.Sequential(
